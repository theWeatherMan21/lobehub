# Client Data 客户端数据层与 Home 数据迁移规范
...
本规范将 SWR 限定为请求调度器，并建立以实体、fragment、索引和快照为核心的客户端数据层。

## 2. 目标与非目标

### 2.1 目标

- 每个 scope 内，同一种实体和 ID 只存在一个可写的 canonical `EntityRecord`。
- 完整 DTO、局部 DTO、mutation 结果和实时事件统一转换为 `DataCommit`。
- fragment 以整体为最小替换单元，不执行无来源语义的 `merge(Partial<Entity>)`。
- 列表只持有实体引用、顺序、成员关系、查询签名和 coverage，不持有实体副本。
- `EntityView` 明确表示 “满足某个已声明 View Contract 的完整视图”。
- 持久化以业务数据模型为单位，不以 SWR key 为单位。
- 已登录 warm reload 在认证确认后可直接使用本地实体数据，并在后台重新验证。
...
| EntityRecord     | scope 内某个 kind/id 的唯一 fragment 容器；允许尚未完整 hydration。                       |
...
| EntityView       | 以一个 root EntityRecord 为主体，可组合引用记录，并满足某个 View Contract 的完整值。      |
| FullEntityView   | 覆盖该实体所有 canonical fragments 的可选视图，不是 `EntityView` 的默认含义。             |
| ClientDataCommit | 一次原子内存变更，包含 fragment replacements、index/snapshot replacements 和 tombstones。 |
...
flowchart LR
  ...
  subgraph Domain["Client Data 数据层"]
    Ingestor["Typed Ingestor"]
    Commit["ClientDataCommit"]
    Graph["Scoped Identity Map / Entity Graph"]
    View["EntityView Selectors"]
  end

  subgraph Persistence["持久化层"]
    Repository["Typed Entity Repository"]
...
Client Data 是全部客户端业务实体的 canonical 数据基础设施，不是与 Home、Chat、Task 并列的
业务 Store。代码放在独立的 `src/client-data` 顶层目录：

```text
src/client-data/
├── core/                 # scope、commit reducer、hydration、通用校验
├── entities/             # canonical entity action、selector、validator
├── modules/
│   └── home/             # Home ingestor、Index、View selector、request hook
├── persistence/          # typed repository 与 localDatabase adapter
├── registry.ts           # 组合实体与业务模块 validator
├── store.ts              # composition root；组合 core/entities/modules
└── index.ts              # 面向消费者的公共入口

src/store/                # 各业务模块的 UI、交互和流程状态
packages/types/src/client-data/
├── entities/             # 按实体拆分的 canonical fragments/records
...
```mermaid
flowchart LR
  Feature["Feature / Business Store"] --> Module["client-data/modules/*"]
  Module --> Entity["client-data/entities"]
  Module --> Core["client-data/core"]
  Entity --> Core
  Core --> Persistence["client-data/persistence"]
```

- `core` 不得依赖 Home、Chat、Task 等业务模块。
- concrete repository 由 composition root 注入 `core`；`core` 不得通过 persistence 反向加载
  任何业务 validator。
- canonical record 按实体命名，例如 `AgentEntityRecord`、`TopicEntityRecord`；不得使用
  `HomeAgentRecord` 之类的 feature-scoped 名称。
- Home 语义仅允许出现在 `home.*` Index、Home Snapshot、View Contract、Ingestor 和请求 Hook。
- `src/store/*` 可以向 Client Data 提交 mutation commit，但不得成为 canonical entity source。
- 新业务模块接入时扩展 Entity/Index/Snapshot registry，不创建第二个 identity map。

### 4.2 React 订阅边界

Entity Graph 的规范化不能止于 Store 内部；React 消费方式同样属于架构契约：

```mermaid
flowchart LR
  RequestHook["Request Hook"] -->|"error / loading / revalidate"| List
  Index["Index Selector"] -->|"ordered entity IDs / refs"| List["List Container"]
  List -->|"entity ID + UI-only props"| Row["Entity Row"]
  Record["Record Selector"] -->|"one EntityView"| Row
```

- Request Hook 只负责请求编排和 coverage 状态，不返回实体数组或聚合 `EntityView`。
- 列表容器只订阅对应 Index，并向行组件传递 entity ID/ref 与事件回调等 UI 参数。
- 行组件按 ID 订阅自己的 EntityRecord，并在本地组装对应 `EntityView`。
- 祖先组件不得把实体数组、完整 read model 或 ClientDataStore 实例作为 props 逐层传递。
- Selector 必须订阅所需的精确 Index、Record 或 fragment；不得为了组装一个列表而订阅整个
  `state.scopes[scope]`。
- Store 不保存预组装的 `EntityView[]`。Index、Record 更新后，Selector 是唯一的视图组装入口。
...
| Identity Map     | scope 内唯一实体记录、commit 冲突处理、原子发布   | 远端事实存储                     |
| View Selector    | 检查 coverage 并组装不可写视图                    | 保存第二份实体数据               |
...
- Anonymous scope 不持久化 Home 私有实体。
- `lobehub:active-scope` 仅可用于提前定位候选分区，不能作为认证证明。
- Web 必须等待 auth session settled；Desktop 必须等待 `isUserStateInit`。只有确认后的 scope
  可以提交或展示持久化 Home 数据。

## 6. EntityRecord 与 fragment 规则

概念类型如下：

```ts
interface EntityFragment<T> {
  data: T;
  observedAt: number;
  source: 'network' | 'mutation' | 'realtime';
}

interface EntityRecord<K extends string, F extends Record<string, unknown>> {
  fragments: { [P in keyof F]?: EntityFragment<F[P]> };
  id: string;
  kind: K;
  tombstoneAt?: number;
}
...
Index 不得嵌入 Agent、Topic、Task 或 Brief 对象。成员被列表移除不等于实体被删除。
...
Snapshot 仍然是 typed、versioned、scoped 的持久化对象，但不参与 Identity Map 去重。

## 9. EntityView 完整性

`EntityView` 不是 “数据库表所有字段的完整实体”，而是 “对一个声明过的 View Contract 完整”。
...
对象。对可选关联实体缺失时，只能省略 enrichment，不能把整个主实体判为不完整。

## 10. Home 第一阶段数据模型

### 10.1 Entities

| Entity    | Fragment      | 字段责任                                                 |
| --------- | ------------- | -------------------------------------------------------- |
...
| Index                   | 内容                                                                              |
| ----------------------- | --------------------------------------------------------------------------------- |
| `home.sidebar`          | pinned/private/grouped/ungrouped 的 entity refs、folder 元数据、pin/unread 上下文 |
...
| Workspace member profile | 当前已有独立 workspace hook；后续迁移为 User entity + membership index |
| Recommendations          | 独立 snapshot，非 Home 首屏正确性的阻塞项                              |
| Document recents         | 当前 Dashboard 只请求 topic；全局 Recents 迁移时加入 Document entity   |
...
| ------------------ | ---------------------------------------- | -------------------------------- |
| `entity-records`   | `${encodedScope}::${kind}::${encodedId}` | versioned EntityRecord envelope  |
...
- hydration 必须运行 Entity、Index、Snapshot 的领域 validator；无效记录按 cache miss 处理。
- schema migration 通过新增 parser/migrator 完成，不修改已发布的底层数据库 migration 历史。
- scope hydration 使用 prefix scan，只读取目标 scope。
- index replacement 与其本次 upsert 的 entity records 必须进入同一个 batch。
...
flowchart TD
  ...
  C --> D["生成 ClientDataCommit"]
  D --> E["Identity Map 原子发布"]
...
- UI 不读取 SWR response 作为数据源，只读取 EntityView selector。
- 请求 Hook 不读取或返回 EntityView；请求状态与事实数据 selector 必须分离。
- Fetcher 在写入 Entity Graph 后只向 SWR 返回轻量 request marker，不把 DTO 留在 SWR cache。
- SWR error 表示本次重新验证失败；若 index 已 hydration，UI 保留 stale EntityView。
...
1. 构造 mutation `ClientDataCommit`，替换明确 fragment 或 index。
2. 原子更新 Entity Graph，并写入 typed repository。
3. 发起服务端 mutation。
...
## 15. Client Data 与业务 Store 的迁移边界

新的 Entity Graph 是 Home Dashboard 的 canonical read source。既有 Store 按以下原则过渡：

- Home 新 UI 不再以 BriefStore/TaskStore/SWR response 为 canonical data。
- 既有 Store 可以暂时保留页面级编辑状态、mutation 状态和旧消费者。
- 兼容写路径必须同时转换为 Entity Graph commit；不得新增反向的 Entity Graph → SWR 数据镜像。
- 旧 Store 中的列表 DTO 属于迁移期 projection，不得被新的 Home 代码直接消费。
- 兼容 projection 可以通过 Store 的非 React subscription 更新；不得要求新组件订阅聚合
  EntityView 来驱动该 projection。
- Agent 列表仍有非 Home 旧消费者，因此保留一个由 `HomeSidebarAgentView` 单向生成的只读
  HomeStore projection；它不自行请求、不接受 partial entity 写入，并在 scope 改变且目标
  EntityView 尚未准备完成时于浏览器绘制前清空。
...
- Index replacement 不立即删除未引用实体，因为其他 index 可能仍引用它。
- Tombstone 和长期无引用实体由后续 GC 清理；首版允许保留，以正确性优先。
- GC 必须按 scope 执行，并在删除前计算所有 index refs。
- logout/expired session 后不展示旧 scope；保留的分区数据只能在同一用户再次认证后读取。
- 不将 auth token、cookie、provider credential 或完整用户设置写入实体层。
...
- 同 kind/id 的多个来源只产生一个 EntityRecord。
- index 只保存 refs，不保存实体对象。
- 带参数的 index 必须验证 coverage；较小的 recent-topic `limit` 不满足较大的 View 请求。
- 早请求晚返回不能覆盖更晚 mutation。
- tombstone 阻止旧响应复活实体。
- EntityView 缺 fragment 时不返回伪完整对象。

### 18.2 Repository 测试

- scope 前缀隔离。
- entity/index/snapshot 在一个 batch 中写入。
...
1. 上述六个 Home 数据集均从 Entity Graph selector 读取。
...
8. Home 列表容器只订阅 Index，实体行只订阅自身 Record；不得通过 props 下发聚合 read model。
9. `src/client-data/core` 不依赖具体业务模块；模块只通过 composition root 接入同一数据层。
