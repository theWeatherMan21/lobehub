...
| DataCommit     | 一次原子内存变更，包含 fragment replacements、index/snapshot replacements 和 tombstones。 |
...
- 祖先组件不得把实体数组、完整 read model 或 EntityStore 实例作为 props 逐层传递。
...
| `HomeRecentTopicView`  | Topic `display`、`activity`、`routing`、`recentNavigation`                                     |
| `HomeInboxTopicView`   | Topic `display`、`activity`、`routing`、`status`、`runTiming`、`homePreview`                   |
...
| Topic     | `recentNavigation` | routePath                                                |
| Topic     | `status`           | status                                                   |
| Topic     | `runTiming`        | runStartedAt                                             |
| Topic     | `homePreview`      | lastAssistantMessage、trigger、userId                    |
...
1. 构造 mutation `DataCommit`，替换明确 fragment 或 index。
