# Agent Topic 数据迁移 Client Data 规范

> 状态：设计定稿，待实施\
> 前置依赖：[Client Data 客户端数据层规范](./client-data-store.zh-CN.md)（fix/home-startup-cache）\
> 交付方式：stacked 分支叠在 fix/home-startup-cache 上，分期 PR

## 1. 背景

Home 第一阶段建立了以实体、fragment、索引为核心的 Client Data 数据层。本规范将 agent
topic 相关数据面迁入同一数据层，并併入 Home 第一阶段遗留的投影清偿（§2.1）；此后的
实体域与基础设施工作收录于 §10 后续路线图。当前一个 topic 实体最多同时存在于 9 个位置：
...
| 8   | client-data repository `entity-records` | canonical 持久化（当前仅 Home 喂入的 fragments）         |

### 1.3 Canonical（覆盖不全）

| #   | 位置                     | 说明                                                                      |
| --- | ------------------------ | ------------------------------------------------------------------------- |
| 9   | client-data Topic record | 仅由 Home ingestor 与 rename/status 兼容写入喂数据，chat fetch 结果未进入 |
...
- chat 三个数据面（sidebar 列表、管理页、搜索）的实体数据全部进入 canonical Entity Graph。
...
- 最终形态：1 份内存 canonical + 1 份磁盘 canonical，列表与搜索只持 refs。
- 併入 Home 第一阶段遗留的投影清偿：
  - HomeStore agent-list projection 的剩余消费者（实测 25 个源文件、10 个 selector 成员，
    含 home 侧栏路由树）翻转到 client-data view，删除 projection 字段与同步链路
    （Client Data 规范 §15 的既定收尾）。
...
- 查询签名不匹配即 coverage 无效，触发重取。两个 index 引用同一批 Topic record，实体字段
...
必需 fragment 缺失按 Client Data 规范 §16 语义视为本地数据损坏：不伪造对象，触发重新验证。
...
等）与 fetch onData 直接 set 点（约 8 处）在同一 PR 内转为 graph commit，遵循 Client Data
规范 §14：
...
- 乐观创建（createTopic）的 commit 必须一次写全 `ChatTopicListItemView` 的必需 fragments，
  否则投影的 coverage 检查会把新行判为损坏并触发多余重取。
...
不是实体数据，留在 chat slice，与 `items` / `total` 结构解耦。`hasMore` 改由 index 的
`total > coveredCount` 派生。

## 5. 投影桥（Phase 1 兼容层）

- client-data store 的非 React subscription（复刻 agentList projection 模式）：commit 触及
...
Phase C 的消费者翻转遵循 Client Data 规范 §4.2 订阅边界：列表容器订阅 index，行组件按
...
- `ordering` fallback：`chatTopicRecord` 对缺失 `sortUpdatedAt` 回退 `updatedAt` 时间戳，
  ...
  改为条件写入。
- `EntityObservation` 与 `fragment` helper 从 `modules/home` 抽到 core/types（消除
  module→module 依赖）；`normalizeChatTopicsSignature` 需从 `src/client-data/index.ts`
  导出供 fetcher 做签名比较。
...
| Phase D/E 后 | Home agent/brief 投影删除；#4 的 topic 行订阅 canonical record。剩 1 内存 + 1 磁盘；#4/#7 的 document/page 快照留待 Document/Page entity（§10） |
...
1. chat 三个数据面的实体数据均从 Entity Graph 读取（Phase B 经投影，Phase C 直读）。
...
| Agent 完整配置（`agent:config` / `available` / `search`）                            | SWR + agent store      | Agent entity 现仅 Home 的 5 个轻 fragments                            |
| Task 列表 / 详情（`task:`）                                                          | SWR + task store       | entity 已有 5 fragments，模式成熟，适合下一个迁                       |
| ChatGroup（`group:detail` / `group:list`）                                           | SWR + agentGroup store | entity 已有 identity/access                                           |
| Document / Page / Notebook（`document:` / `page:` / `notebook:` / `agent:document`） | SWR + 各自 store       | 实体全新建模；完成后解锁全局 Recents 全量迁移与 `recent:*` 持久化退役 |
| Workspace member profile                                                             | 独立 hook              | 规划为 User entity + membership index（Client Data 规范 §10.4）       |
| thread / topicComment                                                                | 各自 store             | chat 域剩余实体                                                       |
| `taskTemplate:` / `modelConfig:` / recommendations                                   | localStorage 壳子      | 建模为 snapshot 即可，低优先                                          |

明确不迁（by design）：auth/user session、用户设置、credentials（Client Data 规范
§10.4 / §17）。

### 10.2 基础设施

- GC：tombstone 与无引用实体按 scope 清理（Client Data 规范 §17 首版故意不做）；实体量
  随迁移增长，宜在两三个实体域迁移后排入。
...
- Realtime 事件直接进 ingestor：现多数实时更新靠 revalidate 兜底，逐步改为事件直接
  commit。
- Home 侧栏行级订阅化：PR D 后侧栏路由树仍是 buckets 聚合订阅，按 Client Data 规范
