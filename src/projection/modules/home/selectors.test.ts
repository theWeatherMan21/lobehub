...
describe('Home Projection selectors', () => {
  ...
  it('does not expose a partial Projection as a complete View Contract', () => {
    const scope = applyProjectionCommit(
      undefined,
      ingestHomeSidebar(sidebarResponse, networkObservation),
    );
    const record = scope.records.agent['agent-1'];
  ...
  it('updates every Brief enrichment through the single canonical Agent record', () => {
    const initial = applyProjectionCommit(undefined, ingestHomeBriefs([brief], networkObservation));
    const renamedAgent: AgentProjection = {
      fragments: {
        identity: {
    ...
    const updated = applyProjectionCommit(initial, { records: [renamedAgent] });
  ...
  it('represents fetched empty coverage as an initialized empty view', () => {
    const scope = applyProjectionCommit(undefined, ingestHomeInboxTopics([], networkObservation));
  ...
  it('filters a tombstoned Projection from an older persisted index', () => {
    const initial = applyProjectionCommit(undefined, ingestHomeBriefs([brief], networkObservation));
    const deleted = applyProjectionCommit(initial, {
