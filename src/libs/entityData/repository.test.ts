...
import {
  createEntityDataRepository,
  ENTITY_DATA_COLLECTIONS,
  ENTITY_DATA_SCHEMA_VERSION,
  entityDataStorageKeys,
} from './repository';
...
const repository = createEntityDataRepository<TestEntity, TestIndex, TestSnapshot>({
  isEntity: (value): value is TestEntity =>
    Boolean(value) &&
    typeof value === 'object' &&
    (value as TestEntity).kind === 'thing' &&
    typeof (value as TestEntity).id === 'string' &&
    typeof (value as TestEntity).value === 'string' &&
    ((value as TestEntity).updatedAt === undefined ||
      (value as TestEntity).updatedAt instanceof Date),
...
describe('createEntityDataRepository', () => {
  ...
  it('persists one logical entity commit as one durable batch', async () => {
    const batch = vi.spyOn(adapter, 'batch');
    const entity: TestEntity = { id: '1', kind: 'thing', value: 'Entity' };
  ...
  it('preserves durable commit order within one scope', async () => {
    ...
    const first = repository.commit('user-1:workspace-1', {
      entities: [{ id: '1', kind: 'thing', value: 'Older' }],
    });
    await Promise.resolve();
    const second = repository.commit('user-1:workspace-1', {
      entities: [{ id: '1', kind: 'thing', value: 'Newer' }],
    });
    await Promise.resolve();
  ...
  it('round-trips structured values without JSON degradation', async () => {
    ...
    await repository.commit('user-1:personal', {
      entities: [{ id: '1', kind: 'thing', updatedAt, value: 'Dated entity' }],
    });

    const hydrated = await repository.hydrateScope('user-1:personal');
    expect(hydrated.entities[0].updatedAt).toBeInstanceOf(Date);
    expect(hydrated.entities[0].updatedAt).toEqual(updatedAt);
  });
  ...
  it('ignores incompatible or invalid persisted envelopes during hydration', async () => {
    ...
    await localDatabase.set(
      ENTITY_DATA_COLLECTIONS.entities,
      entityDataStorageKeys.entity(scope, 'thing', 'old'),
      {
        schemaVersion: ENTITY_DATA_SCHEMA_VERSION + 1,
        value: { id: 'old', kind: 'thing', value: 'Old schema' },
      },
    );
    await localDatabase.set(
      ENTITY_DATA_COLLECTIONS.entities,
      entityDataStorageKeys.entity(scope, 'thing', 'invalid'),
      {
