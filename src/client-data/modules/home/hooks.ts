...
import { clientDataKeys, homeKeys } from '@/libs/swr/keys';
...
import { buildAccountClientDataScope } from '../../core/scope';
...
export interface HomeDataRequest {
  ...
  mutate: KeyedMutator<ClientDataRequestMarker>;
}
...
const marker = (observedAt: number): ClientDataRequestMarker => ({ observedAt });

export const useHomeSidebarRequest = (
  isLogin: boolean | undefined,
): HomeDataRequest & { scope: string } => {
  const scope = useCacheScope();
  const isInitialized = useClientDataStore((state) =>
    Boolean(selectHomeSidebar(state.scopes[scope])),
  );

  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.sidebar(scope) : null,
    async () => {
      const observedAt = Date.now();
      const response = await homeService.getSidebarAgentList();
      getClientDataStoreState().ingestHomeSidebar(scope, response, observedAt);
...
export const useHomeRecentTopicsRequest = (
  ...
  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.recentTopics(scope, limit, view) : null,
    async () => {
      const observedAt = Date.now();
      ...
      getClientDataStoreState().ingestHomeRecentTopics(scope, items, limit, view, observedAt);
      return marker(observedAt);
    },
    { revalidateOnFocus: false },
  );
...
export const useHomeInboxTopicsRequest = (
  ...
  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.inboxTopics(scope) : null,
    async () => {
      const observedAt = Date.now();
      ...
      getClientDataStoreState().ingestHomeInboxTopics(scope, items, observedAt);
      return marker(observedAt);
    },
...
export const useHomeTasksRequest = (isLogin: boolean | undefined): HomeDataRequest => {
  ...
  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.tasks(scope) : null,
    async () => {
      const observedAt = Date.now();
      const result = await taskService.list({});
      getClientDataStoreState().ingestHomeTasks(scope, result.data, result.total, observedAt);
      return marker(observedAt);
    },
    { revalidateOnFocus: false },
  );
...
export const useHomeBriefsRequest = (isLogin: boolean | undefined): HomeDataRequest => {
  ...
  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.briefs(scope) : null,
    async () => {
      const observedAt = Date.now();
      const result = await briefService.listUnresolved();
      getClientDataStoreState().ingestHomeBriefs(scope, result.data, observedAt);
      return marker(observedAt);
    },
    { revalidateOnFocus: false },
  );
...
export const useHomeDailyBriefData = (
  ...
  const accountScope = userId ? buildAccountClientDataScope(userId) : activeScope;
  const snapshot = useClientDataStore(
  ...
  const request = useClientDataSWR<ClientDataRequestMarker>(
    ...
    async () => {
      ...
      getClientDataStoreState().ingestHomeDailyBrief(accountScope, response, observedAt);
      return marker(observedAt);
    },
  );
  return {
