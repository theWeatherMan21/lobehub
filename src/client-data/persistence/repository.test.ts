...
import {
  CLIENT_DATA_COLLECTIONS,
  CLIENT_DATA_SCHEMA_VERSION,
  clientDataStorageKeys,
...
const repository = createClientDataRepository<TestEntity, TestIndex, TestSnapshot>({
...
describe('createClientDataRepository', () => {
  ...
  it('ignores incompatible or invalid persisted envelopes during hydration', async () => {
    ...
    await localDatabase.set(
      CLIENT_DATA_COLLECTIONS.entities,
      clientDataStorageKeys.entity(scope, 'thing', 'old'),
      {
        schemaVersion: CLIENT_DATA_SCHEMA_VERSION + 1,
        value: { id: 'old', kind: 'thing', value: 'Old schema' },
      },
    );
    await localDatabase.set(
      CLIENT_DATA_COLLECTIONS.entities,
      clientDataStorageKeys.entity(scope, 'thing', 'invalid'),
      {
