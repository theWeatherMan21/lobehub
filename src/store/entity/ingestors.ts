...
): HomeAgentRecord | HomeChatGroupRecord => {
  if (item.type === 'group') {
    return {
      fragments: {
...
const recentTopicRecord = (
  ...
  fragments: {
    ...
    recentNavigation: fragment({ routePath: item.routePath }, observation),
...
const taskRecord = (item: TaskListItem, observation: EntityObservation): HomeTaskRecord => ({
  fragments: {
    assignment: fragment(
      {
...
const briefRecord = (item: HomeBriefInput, observation: EntityObservation): HomeBriefRecord => ({
  fragments: {
...
const briefTaskRecords = (
  ...
  const fragments: HomeTaskRecord['fragments'] = {};
