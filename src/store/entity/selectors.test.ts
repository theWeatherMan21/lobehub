...
describe('Home EntityView selectors', () => {
  it('assembles a complete sidebar view from fragments and index references', () => {
    const scope = applyHomeDataCommit(
      undefined,
  ...
  it('updates every Brief enrichment through the single canonical Agent record', () => {
    const initial = applyHomeDataCommit(undefined, ingestHomeBriefs([brief], networkObservation));
    const renamedAgent: HomeAgentRecord = {
      fragments: {
        identity: {
    ...
    const updated = applyHomeDataCommit(initial, { entities: [renamedAgent] });
  ...
  it('represents fetched empty coverage as an initialized empty view', () => {
    const scope = applyHomeDataCommit(undefined, ingestHomeInboxTopics([], networkObservation));
  ...
  it('filters a tombstoned entity from an older persisted index', () => {
    const initial = applyHomeDataCommit(undefined, ingestHomeBriefs([brief], networkObservation));
    const deleted = applyHomeDataCommit(initial, {
